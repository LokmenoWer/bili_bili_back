...
impl Packet {
    ...
    pub fn into_value(self) -> anyhow::Result<PacketValue> {
        Ok(match self.version {
            ...
            BODY_PROTOCOL_VERSION_DEFLATE => {
                PacketValue::Packets(decode_packets(&zlib_decode_bytes(&self.data)?)?)
            }
            BODY_PROTOCOL_VERSION_BROTLI => {
                PacketValue::Packets(decode_packets(&brotli_decode_bytes(&self.data)?)?)
            }
    ...
    pub fn into_messages(self) -> anyhow::Result<Vec<Message>> {
        Ok(self
            .into_raw_messages()?
            .into_iter()
            .filter_map(|v| v.into_message().ok())
            .collect())
    }
...
impl RawMessage {
    pub fn into_message(self) -> anyhow::Result<Message> {
        Ok(match self.op {
            ...
            OP_HEARTBEAT_REPLY => Message::OpHeartbeatReply(OpHeartbeatReply {
                popularity: read_u32(&self.data)?,
            }),
            OP_MESSAGE => {
            ...
            OP_USER_AUTHENTICATION => {
                Message::OpUserAuthentication(Box::new(serde_json::from_slice(&self.data)?))
            }
...
impl Message {
    pub fn to_raw_message(&self) -> anyhow::Result<RawMessage> {
        Ok(match self {
            Message::OpHeartbeat => RawMessage {
                op: OP_HEARTBEAT,
                data: Default::default(),
            },
            Message::OpHeartbeatReply(v) => RawMessage {
                op: OP_HEARTBEAT_REPLY,
                data: v.popularity.to_be_bytes().into(),
            },
            Message::OpMessage(v) => RawMessage {
                op: OP_MESSAGE,
                data: serde_json::to_vec(v)?,
            },
            Message::OpUserAuthentication(v) => RawMessage {
                op: OP_USER_AUTHENTICATION,
                data: serde_json::to_vec(v)?,
            },
            Message::OpConnectSuccess(v) => RawMessage {
                op: OP_CONNECT_SUCCESS,
                data: serde_json::to_vec(v)?,
            },
        })
    }
...
impl Client {
    async fn heartbeat<W: AsyncWrite + Unpin>(
        mut w: W,
        duration: Duration,
        is_run: Arc<AtomicBool>,
    ) -> anyhow::Result<()> {
    ...
    pub async fn new<A: ToSocketAddrs>(
        ...
        // 进房
        Packet::new_user_authentication(room_id, uid, token)?
    ...
    pub async fn new_with_config(
        ...
        let item = config
            ...
            .host_server_list
            .get(0)
            .ok_or(anyhow!("empty host_server_list"))?;
    ...
    pub async fn new_anonymous(room_id: i32) -> anyhow::Result<Self> {
        Self::new(
            room_id,
            "broadcastlv.chat.bilibili.com:2243".to_string(),
            0,
            "".to_string(),
        )
        .await
    ...
    pub async fn next(&mut self) -> anyhow::Result<Message> {
        ...
        Ok(self
            .queue
            .pop_front()
            .expect("fill_queue must wait the queue with at lease"))
    }
}
...
impl ApiClient {
    pub fn new(sess_data: String, bili_jct: String) -> Self {
        let jar = Jar::default();
        jar.add_cookie_str(
            &format!("SESSDATA={}", sess_data),
            &"https://api.live.bilibili.com".parse().unwrap(),
        );
        jar.add_cookie_str(
    ...
    pub async fn send_danmaku(
        ...
        room_id: i32,
        msg: String,
    ) -> anyhow::Result<SendDanmakuResponse> {
        Ok(self
            .client
...
pub mod messages {
    pub mod danmu_msg {
        ...
        impl SEND_GIFT {
            ...
            pub fn gift_price(&self) -> i64 {
                if let Some(v) = self.info.get(0) {
                    if let Some(v) = v.get(5) {
                        if let Some(v) = v.as_i64() {
                            return v;
                        }
                    }
        ...
        #[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]
        pub struct Gift_Data {
            pub action: String,
            pub bag_gift: Value,
            pub batch_combo_id: String,
            pub batch_combo_send: BatchComboSend,
            #[serde(rename = "beatId")]
            pub beat_id: String,
            pub biz_source: String,
            pub blind_gift: Value,
            pub broadcast_id: i64,
            pub coin_type: String,
            pub combo_resources_id: i64,
            pub combo_send: ComboSend,
            pub combo_stay_time: i64,
            pub combo_total_coin: i64,
            pub crit_prob: i64,
            pub demarcation: i64,
            ...
            pub draw: i64,
            pub effect: i64,
            pub effect_block: i64,
            pub face: String,
            pub face_effect_id: i64,
            pub face_effect_type: i64,
            pub float_sc_resource_id: i64,
            #[serde(rename = "giftId")]
            pub gift_id: i64,
            ...
            pub remain: i64,
            pub rnd: String,
            pub send_master: Value,
            pub silver: i64,
            #[serde(rename = "super")]
            pub super_field: i64,
            pub super_batch_gift_num: i64,
            pub super_gift_num: i64,
            pub svga_block: i64,
            pub switch: bool,
        ...
        #[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]
        pub struct INTERACT_WORD {
            pub cmd: String,
            pub data: INTERACT_WORDData,
        }
        #[derive(Default, Debug, Clone, PartialEq, Serialize, Deserialize)]
        pub struct INTERACT_WORDData {
            ...
            pub roomid: i64,
            pub score: i64,
            pub spread_desc: String,
            pub spread_info: String,
            pub tail_icon: i64,
            pub timestamp: i64,
